...
import { GroupByHotelDto } from '../../../../../core/models/hotel/GroupByHotelDto';
import { HotelService } from '../../../../../core/services/api/hotel.service';
import { GroupByHotelResponse } from '../../../../../core/models/hotel/GroupByHotelResponse';
...
export class HotelComponent {
  ...
  loadInitialReservations() {
    ...
    this.hotelService.getReservationsGroupedByHotel("TotalSalesPrice", "Ok", true, 0, 5).subscribe({
      next: (response: GroupByHotelResponse) => {
  ...
  loadReservations(event: TableLazyLoadEvent): void {
    ...
    this.hotelService.getReservationsGroupedByHotel("TotalSalesPrice", "Ok", true, page, size).subscribe({
      next: (response: GroupByHotelResponse) => {
        this.reservations = response.data;
        this.totalRecords = response.totalCount;
        this.updateChartData(response.data);
  ...
  updateChartData(data: GroupByHotelDto[]): void {
    this.chartData.labels = data.map((d) => d.hotelName );
